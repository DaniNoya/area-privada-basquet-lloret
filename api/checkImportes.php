<?php
    ...
    if ($jugadorMas18 == true) {
        ...
        $isSocioClub = false;
        $querySocio = "SELECT * FROM persona WHERE dni = '$dni' AND id IN(SELECT id_persona FROM socio WHERE id IN(SELECT id_socio FROM socio_temporada WHERE id_temporada = (SELECT id FROM temporada WHERE id = 6)));";
        ...
        $isSocioClub = false;
        $querySocio = "SELECT * FROM persona WHERE dni = '$dniT' AND id IN(SELECT id_persona FROM socio WHERE id IN(SELECT id_socio FROM socio_temporada WHERE id_temporada = (SELECT id FROM temporada WHERE id = 6)));";
        if ($resultQuerySocio = mysqli_query($con, $querySocio)) {
            if ($resultQuerySocio->num_rows > 0) {
                $isSocioClub = true;
            }
    ...
    //$importe->importe = $precioUnitario - $precioDescunetEsSocio;
    ...
    if($isSocioClub == true){$precioDescunetEsSocio = (($precioUnitario * $porcentajeDescuentoEsSocio) / 100);}
    ...
    if($importe->porcentajeDescuento == "100"){
        ...
        $precioTotalPagar = ($precioUnitario - $precioDescunetPagoUnico) - $precioDescunetEsSocio;
        $restante = 0;
        $importe->restante = $restante;
    } else if ($jugadorMas18 == false) {
        $precioTotalPagar = (($precioUnitario - $precioDescunetPagoUnico) - $precioDescunetHermano) - $precioDescunetEsSocio;
        $restante = 0;
        $importe->restante = $restante;
    }
