...
function obtenerImportes($con, $concepto) {
    $sql = "SELECT *,
                   (SELECT importe FROM importes WHERE concepto = 'quotaInscripcionFormacion' AND idTemporada = (SELECT MAX(id) FROM temporada)) AS importeQuotaInscripcion,
...
foreach ($registrationPlayers as $player) {
    ...
    $importesConcepto = 'quotaAnualFormacion';
