...
<?php
          ...
          if(strpos($response->resultat, "KO") === false){
              ...
              if ($resultPersona = mysqli_query($con, $sqlConsultaIdPersona)) {
                  ...
                  if ($result = mysqli_query($con, $sqlConsultaJugador)) {
                      if ($result->num_rows > 0) {
                          $sqlUpdateJugador = "UPDATE jugador SET baja = 0, tarjetaSanitaria = '$tsiPlayer'WHERE id = '$idPersonaMas18'";
                          ...
                          $sqlInsertJugador = "INSERT INTO jugador (id, baja, tarjetaSanitaria, escuela, curso) VALUES ('$idPersonaMas18',0,'$tsiPlayer', 'Jugador +18', 'Jugador +18')";
                          if (mysqli_query($con, $sqlInsertJugador)) {
                              $message = "JUGADOR INSERTADO +18";
        ...
        if (strpos($response->resultat, "KO") === false) {
          ...
          $sqlInsertCamisetaJugador = "INSERT INTO jugador_campus_ropa (id, id_jugador, id_campus, talla) VALUES (NULL,(SELECT id FROM persona WHERE dni = '$dniPlayer'),$idTipo_pago,'$tallaRopaCHPlayer')";
          if (!mysqli_query($con, $sqlInsertCamisetaJugador)) {
            $message = "CAMISETA JUGADOR INSERTADA";
            echo '<script>console.log(' . json_encode($message) . ');</script>';
          }else{
            $response->causa = mysqli_error($con);
            echo '<script>console.log(' . json_encode($response->causa) . ');</script>';
          }
            ...
            foreach($arrayJugadores as $k => $value){
              ...
              if(strpos($response->resultat, "KO") === false){
                  ...
                  if ($resultPersona = mysqli_query($con, $sqlConsultaIdPersona)) {
                      ...
                      if ($result = mysqli_query($con, $sqlConsultaJugador)) {
                          if ($result->num_rows > 0) {
                              $sqlUpdateRelacionJugador = "UPDATE jugador SET baja = 0, tarjetaSanitaria = '$tsiJ', escuela = '$schoolJ', curso = '$courseJ' WHERE id = '$idPersonaMenos18'";
                              ...
                              $sqlInsertRelacionJugador = "INSERT INTO jugador (id, baja, tarjetaSanitaria, escuela, curso) VALUES ('$idPersonaMenos18',0,'$tsiJ','$schoolJ','$courseJ')";
