...
switch ($_SERVER['REQUEST_METHOD']) {
  case 'GET':
    ...
    $sqlPayments = "SELECT (SELECT concepto FROM tipo_pago WHERE id = m.tipo_pago) as paymentType, dni,
                          ...
                          (SELECT jt.quota FROM jugador_temporada jt WHERE jt.idJugador = p.id AND jt.idTipo = (SELECT id FROM tipo_pago WHERE id = m.tipo_pago) GROUP BY jt.idJugador) AS total,
