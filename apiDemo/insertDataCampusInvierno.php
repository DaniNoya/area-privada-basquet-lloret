    ...
    if ($result->num_rows > 0) {
      ...
      if ($esMas18 == true) {
        ...
        foreach($dataForm as $k => $value){
          ...
          if ($k == 'form_fields[tallaCamisetaRegaloCHJugador]') {$tallaRopaCHPlayer = $value;}
...
<?php
          ...
          if(strpos($response->resultat, "KO") === false){
              ...
              if ($resultPersona = mysqli_query($con, $sqlConsultaIdPersona)) {
                  ...
                  if ($result = mysqli_query($con, $sqlConsultaJugador)) {
                      if ($result->num_rows > 0) {
                          $sqlUpdateJugador = "UPDATE jugador SET baja = 0, tarjetaSanitaria = '$tsiPlayer', talla_camiseta_regalo_CH = '$tallaRopaCHPlayer' WHERE id = '$idPersonaMas18'";
                          ...
                          $sqlInsertJugador = "INSERT INTO jugador (id, baja, tarjetaSanitaria, talla_camiseta_regalo_CH) VALUES ('$idPersonaMas18',0,'$tsiPlayer','$tallaRopaCHPlayer')";
        ...
        if (strpos($response->resultat, "KO") === false) {
          $peto = false;
          foreach ($campusTurnos as $key => $value) {
            $fechaHoraInicio = explode(' / ', $value)[0];
            $fechaHoraFin    = explode(' / ', $value)[1];

            $sqlInsertJugadorCampusTurno = "INSERT INTO campus_jugador_turno(`id`,`id_campus`,`id_jugador`,`fecha_hora_inicio`,`fecha_hora_fin`) VALUES (NULL,$idTipo_pago,(SELECT id FROM persona WHERE dni = '$dniPlayer'),'$fechaHoraInicio','$fechaHoraFin');";
        
            if (!mysqli_query($con, $sqlInsertJugadorCampusTurno)) {
              $peto = true;
              break;
            }
          }
          ...
          if ($peto == false) {
            $response->resultat = "INSERT_JUGADOR_TURNO_OK";
          } else {
            $response->resultat = "INSERT_JUGADOR_TURNO_KO";
            $response->causa = mysqli_error($con);
          }
        }
          ...
          if ($k == 'Jugadores') {
            foreach($value as $jugador){
              ...
              foreach($jugador as $k2 => $value2){
                ...
                if ($k2 == 'form_fields[tallaCamisetaRegaloCHJugador]') {$arrayJugador['Talla1Ropa'] = $value2;}
              }
            ...
            foreach($arrayJugadores as $k => $value){
              ...
              if(strpos($response->resultat, "KO") === false){
                  ...
                  if ($resultPersona = mysqli_query($con, $sqlConsultaIdPersona)) {
                      ...
                      if ($result = mysqli_query($con, $sqlConsultaJugador)) {
                          if ($result->num_rows > 0) {
                              $sqlUpdateRelacionJugador = "UPDATE jugador SET baja = 0, tarjetaSanitaria = '$tsiJ', escuela = '$schoolJ', curso = '$courseJ', talla_camiseta_regalo_CH = '$talla1RopaJ' WHERE id = '$idPersonaMenos18'";
                              ...
                              $sqlInsertRelacionJugador = "INSERT INTO jugador (id, baja, tarjetaSanitaria, escuela, curso, talla_camiseta_regalo_CH) VALUES ('$idPersonaMenos18',0,'$tsiJ','$schoolJ','$courseJ','$talla1RopaJ')";
              ...
              if(strpos($response->resultat, "KO") === false){
                for ($i=0; $i < count($campusTurnos); $i++) { 
                  $fechaHoraInicio = split(' / ', $campusTurnos[$i])[0];
                  $fechaHoraFin = split(' / ', $campusTurnos[$i])[1];
      
                  $sqlInsertJugadorCampusTurno = "INSERT INTO `campus_jugador_turno`(`id`,`id_campus`,`id_jugador`,`fecha_hora_inicio`,`fecha_hora_fin`) VALUES (NULL,$idTipo_pago,(SELECT id FROM persona WHERE dni = '$dniJ'),'$fechaHoraInicio','$fechaHoraFin');";
                  if (mysqli_query($con, $sqlInsertJugadorCampusTurno)) {
                    $response->resultat = "INSERT_JUGADOR_TURNO_OK";
                  } else {
                    $response->resultat = "INSERT_JUGADOR_TURNO_KO";
