...
switch ($_SERVER['REQUEST_METHOD']) {
  case 'GET':
    ...
    if (isset($params->jugador)) {
      ...
      $sql = "SELECT p.*, j.*, (SELECT sexo FROM sexo WHERE id = p.id_sexo) as sexo, ".
          "(SELECT foto FROM fotos WHERE id_persona = p.id ORDER BY id_temporada DESC LIMIT 1) as foto, (SELECT COUNT(*) FROM movimientos WHERE dniJugador = p.dni) as countMovimientos FROM persona p INNER JOIN jugador j ON p.id = j.id ".
          "WHERE p.id IS NOT NULL AND p.id IN(SELECT idJugador FROM jugador_temporada where idTipo=$conceptoSelecciopnado) ";
      ...
      switch ($estadoJugador) {
        case "inscritos":
          $sql .= "AND p.dni IN (SELECT dniJugador from movimientos where pagoCompletado = 1 OR pagoCompletado = 0 AND pagoManual = 1) ";
          break;
        case "noInscritos":
          $sql .= "AND p.dni IN (SELECT dniJugador from movimientos where pagoCompletado = 0 AND pagoManual = 0) ";
          break;
        /*case "all":
