...
switch ($_SERVER['REQUEST_METHOD']) {
  case 'GET':
    ...
    $sqlPayments = "SELECT (SELECT concepto FROM tipo_pago WHERE id = m.tipo_pago AND idTemporada = (SELECT MAX(id) FROM temporada)) as paymentType, dni,
                          CONCAT(p.nombre, ' ', p.primer_apellido, ' ', p.segundo_apellido) AS nombre,
                          (SELECT jt.quota FROM jugador_temporada jt WHERE jt.idJugador = p.id AND jt.idTipo = (SELECT id FROM tipo_pago WHERE id = m.tipo_pago AND idTemporada = (SELECT MAX(id) FROM temporada))) AS total,
                          (SELECT IFNULL(SUM(importe),0) FROM movimientos WHERE dniJugador = p.dni AND pagoCompletado = 1 AND tipo_pago = (SELECT id FROM tipo_pago WHERE id = m.tipo_pago AND idTemporada = (SELECT MAX(id) FROM temporada))) as pagado
                  FROM movimientos m
    ...
    if ($_GET['paymentType'] == "all") {
      ...
      $sqlPayments .= "WHERE tipo_pago IN (SELECT id FROM tipo_pago WHERE id = '".$_GET['paymentType']."' AND idTemporada = (SELECT MAX(id) FROM temporada))
                       AND pagoManual = 1
